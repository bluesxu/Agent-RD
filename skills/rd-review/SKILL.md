...
### 变异测试 —— 自造对抗性变异，验证测试集够不够用

> 📎 **`task.mutationTargets` 非空时** → 读 `references/mutation-followup.md`
> 不读会：把变异当成例行公事 —— 自造变异体是**唯一直接检验「测试集够不够用」**的手段。
> 漏掉它，下一轮测试层跑全绿也只是「实现和测试一致」，不是「实现正确」。

## 测试作者 —— 你的第二职责

**你既是审查者，也是测试作者。**Builder 不写测试，测试层只负责机械跑测试，
「测试集够不够用」这道判定由你负责。

**写测试的时机卡死（不许提前，也不许跳过）**：

1. **blocking 清零**（follow-up 确认之后）
2. **freeze 校验通过**（`freeze-target -Verify` 通过之后）

在这两个条件满足之前**不许写测试** —— 测试要审的是已经稳定下来的代码，
不是还在改的代码。写出来的测试**不进本轮审查目标**，但**进下一轮 freeze**
（下一轮的 `review-target.json` 会把 `.rd/features/{slug}/tests/` 从磁盘收集进来）。
...
- 对 `task.mutationTargets` 声明的文件做**自造对抗性变异**（见变异段），
...
- 修复后代码符合 spec、测试仍红 → **测试错了**，你可以改自己的测试，
  **只朝 checkIntent 靠拢，不许朝实现靠拢**。
- 把测试改成「期望 = 实现当前输出」就是迁就实现 —— 那等于把测试从行为判据
  降级成实现回声，后续所有基于它的判定都建立在回声上。
- 改不到 `-MustMatch` 锚点以下 —— 锚点是 plan 阶段冻结的，锚点以上的测试逻辑
  才是你的自由区。
...
- **只读（开窄口）**：不修业务代码、不改写业务文件、不 commit。修复由 `rd-build` 派 fix agent 负责。
  **唯一例外**：`.rd/features/{slug}/tests/` 下的 AC 测试由你（测试作者）在
  blocking 清零 + freeze 通过之后补写 —— 那是你唯一允许写文件的地方。
  （Go / Rust 例外：测试必须留在包内、随产品代码走 —— 这类语言的包内测试文件也归你补写。）
- **叶子执行器**：禁止创建、委派或唤醒任何子 agent；不得再次调用 `rd-review`；
  不得把审查转交给其他流程。
- **必须返回终态**：每次调用必须给出一份完整审查结果。上下文不足时返回
  `NeedsContext` + 缺什么 + 已检查范围，**不得以空结果或"等待中"结束**。
- **不许顺手修**：业务代码的问题只报告，不当场改；测试只能按上面「测试作者」的红线补写。
