...
if (Test-Path $gatesDst) {
    Write-Host "  exists   .\.workflow\gates.json（跳过，不覆盖）[检测到: $kind]" -ForegroundColor DarkGray
} else {
    ...
    # 而 init-workflow 往往在阶段 0 之前就跑了 —— 那时技术选型还没论证。
    ...
    if (-not $hasMarker) {
        ...
        $out | Add-Member -MemberType NoteProperty -Name '_provisionalNote' -Value "⚠️ 目录里没有任何语言标记文件（Cargo.toml / go.mod / pyproject.toml / requirements.txt / package.json），上面这套门是**猜的**（默认按 node）。技术选型是阶段 1 的事 —— wf-plan 仲裁完之后必须回来改掉这份配置并删除本标记。在此之前，L1 机械门验的是一个还没被决定的技术栈。"
    }
...
# ---- 随项目下发守卫脚本，让项目自包含 ----
# 为什么必须这样：agentflow 装在项目外面，acceptance.json 里的 check 只能用相对路径引它。
...
if (Test-Path $guardSrc) {
    ...
    Write-Host "  vendored .\.workflow\bin\check-ac.ps1" -ForegroundColor Green
...
$common = @('# --- agentflow init ---', '.DS_Store', 'Thumbs.db', '*.log')
switch ($kind) {
...
if (Test-Path $giPath) {
    ...
    if ($missing.Count -gt 0) {
        Add-Content -Path $giPath -Value ("`n# --- agentflow init [$kind] ---") -Encoding UTF8
...
Write-Host ""
Write-Host "  ⛔ 忽略清单里故意不含 .workflow 下的任何东西：" -ForegroundColor DarkGray
