...
- **一条本身就站得住的指令，后面不要再贴励志理由。**「为什么必须写」这类说服性文字
  搬进 `references/rationale.md`，正文只留「做什么 + 什么算做到了」。
...
- **role**：`self` = 执行者自己读；`forward` = 整份转发给子 agent，本体不读正文
  （此时后果子句写「不转发会」）；`human` = 运行期永不加载（`rationale.md` 专用）。
...
- 写作理由文字（O-4）不单独建文件：跟着它解释的块走。只有它解释的规则留在主文件时，
  才并进该 skill 已有的 reference。每个 skill 至多一份 `references/rationale.md`。
...
skills/<skill>/
  ...
  references/
    ...
    rationale.md              # 人读的设计理由，运行期永不加载（role=human）
```
